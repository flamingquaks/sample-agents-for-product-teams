...
| Lambda | `jira-webhook-${Stage}` / `confluence-webhook-${Stage}` | Always deployed, inert until an Atlassian site is onboarded with that product enabled. Verify the per-delivery Forge Invocation Token (RS256 vs Atlassian's JWKS — no webhook secret exists), cross-check the cloud id, dedup, guard the bot loop, scan ADF for mentions, front-load issue/page context, run automation-rule matching, invoke Dispatch Router async (atlassian-connector spec §B1/§C1) |
| Lambda | `jira-broker-${Stage}` / `confluence-broker-${Stage}` | AgentCore Gateway Lambda targets (`JiraTarget`/`ConfluenceTarget`): curated no-delete tool schemas, container allowlists (Confluence reads included), Confluence write-mode/`base_version` enforcement, per-invocation site-token fetch |
| Lambda | `automation` engine (in the receivers) + `atlassian-token-check-${Stage}` | Event→agent automation rules (§A8) matched in the receivers; a scheduled Lambda tracks API-token expiry and fires `credential_expired` notifications (14/3/0 days) |
| API Gateway | `WebhookApi` | Fronts the webhook Lambdas at `/github/webhook`, `/asana/webhook`, `/jira/webhook/{site}`, `/confluence/webhook/{site}`, and (when Slack is enabled) `/slack/events` + `/slack/commands` |
...
**Outputs:** `AssignmentsTableName`, `ArtifactsBucketName`, `DispatchRouterArn`, `WebhookEndpoint` (Asana webhook URL), `GitHubWebhookEndpoint`, `TriggerPolicyStoreId`, `WebhookApiId`, `SlackEventsEndpoint`, `SlackCommandsEndpoint`, `SlackInteractionsEndpoint` (always emitted — set them on the Slack app when you onboard a workspace), `JiraWebhookEndpoint`, `ConfluenceWebhookEndpoint` (wired into the `atlassian-events` Forge forwarder at its deploy time).
...
- **`DeployAgentCoreMemory`** — `true`/`false` (default `true`). Provisions the fleet's shared `AWS::BedrockAgentCore::Memory` (short-term event memory, 90-day event expiry — sized so the reviewer's per-PR review ledger outlives a PR's active life) and injects its id as `AGENTCORE_MEMORY_ID` on every agent runtime; the capability deployer also grants each runtime role the memory data-plane actions scoped to that one Memory. Native AgentCore type — unlike `AWS::BedrockMantle::Project`, **no** `activate-type` step is needed. Set `false` to skip and bring your own id via `AgentCoreMemoryId`; with neither, agents skip loading memory tools and the reviewer's incremental-review ledger no-ops.
- **`AgentCoreMemoryId`** — a pre-existing AgentCore Memory id, used only when `DeployAgentCoreMemory=false`. Blank (default) leaves agents memory-less. Ignored when `DeployAgentCoreMemory=true`.
...
| `/sdlc-agents/${Stage}/atlassian/<site_id>/api-token` | The dashboard **Connect a site** flow (Connectors → Atlassian — the admin API's one `ssm:PutParameter`, scoped to this path) | Jira/Confluence receivers (context fetches), brokers (agent tools), Router (`post_jira_comment`/`post_confluence_comment`), token-expiry check. The ONE Atlassian secret — the Forge event transport has none |
| `/sdlc-agents/${Stage}/atlassian/forge-app-id` + `.../forge-install-link` | `scripts/deploy_forge_atlassian.py` (run by `deploy_fleet.py`) | Admin API `GET /admin/atlassian/forge-status` → the Sites tab's app-install card; receivers pin the app id on first verified delivery |
...
10. **(Optional) Enable Slack** — the receiver is already deployed; run `scripts/bootstrap_slack.py` to write the app signing secret + per-workspace bot token and register the Slack app manifest against the `SlackEventsEndpoint`/`SlackCommandsEndpoint`/`SlackInteractionsEndpoint` outputs, then onboard each workspace/channel in the dashboard Connectors → Slack panel. Users self-serve notifications with `/sdlc-notify` (channels) and `/sdlc-notify me` (personal DMs).
11. **(Optional) Enable Jira/Confluence** — the receivers, brokers, and gateway targets are already deployed and inert. The `atlassian-events` Forge forwarder ships with step 3 when the Forge CLI is logged in (`npm i -g @forge/cli && forge login`; re-run `python scripts/deploy_forge_atlassian.py --stage <stage>` any time). Everything else is in-dashboard: Connectors → Atlassian → Connect a site (paste the service-account email + scoped API token), toggle products, install the Forge app from the install card, Verify delivery, onboard projects/spaces, grant access rules. See `skills/sdlc-agents-connect-atlassian/SKILL.md` for the walk-through. Gate `DeployJiraTarget`/`DeployConfluenceTarget` follow `DeployGateway`.
12. **Verify** — `sdlc-agents-verify` runs layered smoke tests (runtime health → credential freshness → end-to-end mention).
...
2. ~~**AgentCore Memory isn't provisioned.**~~ Closed: the foundation stack now provisions the fleet's shared AgentCore Memory by default (`DeployAgentCoreMemory=true`, § 2.3) and the capability deployer wires `AGENTCORE_MEMORY_ID` + a Memory-scoped IAM grant onto every runtime.
...
6. Delete the SSM parameters (`asana-*`, `github-app-*`, `researcher-tavily-api-key`, and — if Slack/Atlassian were enabled — everything under `/sdlc-agents/${Stage}/slack/*` and `/sdlc-agents/${Stage}/atlassian/*`) and the Secrets Manager `sdlc-agents/github-app/private-key` secret.
...
8. If Atlassian was enabled, uninstall the `atlassian-events` Forge app from each site (silences both products) and optionally `forge uninstall`/delete the app from the developer account; revoke the service account's API token in Atlassian.
9. If you enabled the optional Claude Code on Bedrock feature (§1.4), delete its `ClaudeCodeBedrockRole` and the GitHub OIDC provider/trust you created for it. (The fleet itself creates no OIDC provider or CI role to clean up.)
10. Disable Bedrock model access (optional).
