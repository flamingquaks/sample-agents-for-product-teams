...
**T-11 (Partially mitigated):** The GitHub MCP server is authenticated with either a PAT or a GitHub App installation. The PAT path cannot enforce minimum scopes — a token with `repo` grants write access to every repository the owner can access, not just the onboarded repos. Two controls now constrain the *use* of that token even when it is over-scoped: (1) the multi-repo transformation routes agent tool calls through an **AgentCore Gateway** whose **Cedar policy engine** enforces the admin repo allowlist — a `forbid` on GitHub write tools whose target repo isn't enabled + multi-repo-eligible (`infra/dashboard/fleet_policy.py`, synced by the admin API on every allowlist change). With enforcement `ACTIVE`, a write against a non-allowlisted repo is denied deterministically at the boundary regardless of the PAT's breadth. (2) The Dispatch Router already rejects mentions from non-onboarded repos (`infra/dispatch/fleet_config.py`). The enforcement path is now wired end-to-end: agents SigV4-connect to the gateway with their runtime role (`agents/shared/tools/gateway.py`), and the admin API syncs both the repo-allowlist forbid and per-agent `permit` policies keyed on each runtime role's ARN (`fleet_policy.AGENT_TOOL_GRANTS`), so an `ACTIVE` engine both scopes each agent to its tools and blocks non-allowlisted repos. Residual risk (why not fully Mitigated): the underlying PAT is still over-scoped as defense-in-depth, and the gateway is opt-in (`DeployGateway`) and rolled out `LOG_ONLY` first — until enforcement is `ACTIVE`, deny decisions are logged, not blocked, and the tool names in the policies must be reconciled against the live gateway manifest (`scripts/check_gateway_manifest.py`). Progress toward bounding the credential itself: a **GitHub App** path is now implemented for onboarding — the admin UI registers the App (manifest flow; private key in Secrets Manager, app id/slug in SSM) and, in `GitHubAuthMode=app`, onboarding verifies the App is installed on a repo's owner and can reach the repo before activating it, resolving a **per-owner installation** so one App bounds many individual + org owners (`infra/dashboard/github_client.py`, `docs/specs/github-onboarding-spec.md`). Residual (why still Partially mitigated): the **agent runtime and the dispatch reply Lambda still read the shared PAT** — cutting those call sites over to per-owner App installation tokens (direct mode) and adding the Gateway-mode SCM broker that mints them per call are tracked, not-yet-built steps (spec §3.5–§3.6, §6). Roadmap: complete the agent-side move to App installation tokens so the credential is bounded end-to-end, and default the gateway to `ACTIVE` once the manifest check passes.
