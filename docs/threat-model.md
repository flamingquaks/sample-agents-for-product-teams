...
| C-2b | Discord Webhook Lambda | AWS Lambda + API Gateway | Public HTTPS endpoint; verifies Ed25519 signature; forwards slash commands to Dispatch Router |
...
**T-8b (Mitigated — Discord analogue of T-8):** `infra/dispatch/discord_webhook.py` fetches the Discord public key on demand from SSM per invocation and never caches it in a module-level global. The bot token is not accessed by the receiver at all; only `reply.py` and `agents/shared/discord_post.py` read it (per-invocation and per-container-init respectively). Exposure window is bounded to the single in-flight request for the receiver, and to the container lifetime for the agent tool.

**T-9b (Mitigated — Discord analogue of T-9):** The discord-webhook Lambda's IAM policy grants only `ssm:GetParameter` on the public-key and bot-token parameters — no `ssm:PutParameter`. The application public key and bot token are written to SSM by the operator running `scripts/bootstrap_discord_app.py`, which uses operator credentials, not the Lambda's role. A subsequent attempt to POST a crafted handshake to the endpoint receives 401 (invalid Ed25519 signature) — there is no handshake path that can overwrite SSM from this Lambda.

**T-9c (Mitigated — Discord replay):** Discord does not expose a server-controlled timestamp window in the signed body the way HMAC protocols do; the `X-Signature-Timestamp` value is signed but there is no server-side replay cache. Discord's own docs note that clients may reject requests where the timestamp is more than 5 seconds old, but this is a client-side heuristic, not a server-enforced window. The Ed25519 signature itself is binding — an attacker who intercepts a signed request cannot modify it. The practical replay risk (resending an already-accepted signed request) is low because slash commands are idempotent by design (the agent re-reads current task/issue state before acting). Operators who require strict replay prevention can add a short-TTL DynamoDB cache keyed on `(signature_hex)` in the receiver; this is not implemented in v1.

**T-29 (Discord Gateway-listener future risk — Open):** v1 ships slash commands only; `@mention` events require a long-running Gateway WebSocket listener. A Fargate task holding a bot token is a materially different posture than the per-invocation Lambda: the token is resident in memory continuously, the process is long-lived (not per-request), and a compromised task has persistent access to all MESSAGE_CREATE events in every guild the bot has joined. When the Gateway listener is built, the threat model should add: bot-token rotation policy, structured logging of all received events, least-privilege intent subscription (only MESSAGE_CREATE for guilds where the bot is installed), and a circuit-breaker alarm if the listener reconnects unexpectedly (which can indicate a token compromise).

**T-30 (Discord snowflake sender identity — Mitigated):** Discord user snowflake IDs are immutable and non-self-editable (analogous to Asana GIDs, threat T-4). The receiver reads `interaction.member.user.id` (guild context) or `interaction.user.id` (DM context). A user cannot change their snowflake. The authorization allowlist in `.dispatch/agents.yaml` should be populated with snowflake IDs, not usernames (usernames are mutable on Discord). The receiver logs a warning and sets sender to "unknown" if the ID cannot be resolved; the router's fail-closed behavior (T-4) rejects "unknown" senders even against a wildcard allowlist.
