...
**T-51 (Mitigated):** A paused thread is an open door — anyone who can type in the channel could otherwise answer the agent's question and steer code-writing work. The resume path treats the reply as a **full dispatch**: the router runs the same Cedar trigger-authz on the *replier* (fail-closed on unresolved senders, channel groups stamped so channel grants apply), then the same prompt-injection guardrail on the reply text, and only then takes the `awaiting_input → resuming` **conditional-write lock** (a second concurrent reply loses the condition and is rejected — no double resume). The agent id is resolved from the server-side thread binding, never from the reply text.
...
| **High (Open)** | 1 | T-23 |
| **High (Mitigated)** | 3 | T-32, T-35, T-41 |
| **Medium (Open or Partial)** | 8 | T-11, T-13, T-17, T-21 (open); T-12, T-18, T-22, T-30 (partial) |
| **Medium (Mitigated)** | 10 | T-31, T-33, T-34, T-36, T-37, T-38, T-43, T-44, T-46, T-47 |
| **Medium (Partially mitigated)** | 2 | T-42, T-48 |
...
- Covers the fleet as shipped: four agents (workitems, researcher, docwriter, adr), Dispatch Router, GitHub App + Asana + Slack (always-deployed) webhooks, AVP-backed trigger authorization (`TriggerPolicyStore`), the **cross-source identity map + permission groups** (C-20/C-21), **self-served tiered notifications** (C-22), the AgentCore Gateway (gateway-only tool access), the AVP-authorized dashboard with the Connectors admin panel (incl. the Access/Users/Groups sub-page), and the UI-driven onboarding pipeline (CodeBuild + capability-deployer). AgentCore Memory, AgentCore Identity, and Feedback/UAT agents are out of scope — they are not yet implemented. Slack usergroup *import* is a possible enrichment source for identity groups (not required); shipped group membership is dashboard-assigned at onboarding.
