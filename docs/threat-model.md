...
| C-22 | Notification subscriptions (`notif_sub#` rows + `notify.py` fan-out + `slack_notify.py` modal) | DynamoDB data + Lambda modules | Self-served, tiered Slack notifications (spec §18). A channel configures a subscription via the `/sdlc-notify` Block Kit modal (submitted on `/slack/interactions`); `notify.py` fans fleet + SCM events out to subscribed channels, threaded per unit-of-work, with @mentions (resolved through C-20 to the right Slack user per workspace) only on the `actionable`/`error` tiers. Repo scope is bounded to the fleet's onboarded repos. A subscription only *receives* — it grants no access, so it needs no admin approval |
