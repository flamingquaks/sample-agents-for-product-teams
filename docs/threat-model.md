...
| C-22 | Notification subscriptions (`notif_sub#` rows + `notify.py` fan-out + `slack_notify.py` modal + `assignment_notifier.py` stream Lambda) | DynamoDB data + Lambda modules | Self-served, tiered Slack notifications (spec §18). A channel configures a subscription via the `/sdlc-notify` Block Kit modal (submitted on `/slack/interactions`); `notify.py` fans events out to subscribed channels, threaded per unit-of-work, with @mentions (resolved through C-20 to the right Slack user per workspace) only on the `actionable`/`error` tiers. Three event origins: the **Router** (dispatch-time: run_started/failed, guardrail_tripped), the **GitHub webhook** (SCM: PR opened/merged/review-requested, issue opened), and the **assignments DynamoDB stream** (`assignment-notifier` Lambda — agent-side completed/failed/awaiting_approval, so agents need no Slack credential). Repo scope is bounded to the fleet's onboarded repos. A subscription only *receives* — it grants no access, so it needs no admin approval |
...
**T-42 (Partially mitigated):** Lazy creation from email-less sources means one person can briefly hold two records. Auto-merge fires **only on an exact email match** (the strongest signal); the union keeps a `merged_from` audit trail so a bad merge is visible and reversible. Weaker signals (handle/display-name collisions) are **not** auto-merged — they surface for admin review. Residual: an attacker who can make a source assert a victim's email (e.g. a spoofed webhook) could trigger a wrong merge — but every source's email is only trusted post-signature-verification (Slack `users.info`, the signed GitHub/Asana events), so an unauthenticated actor can't inject an email claim.
