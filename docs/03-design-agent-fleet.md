...
**`fleet-config` kind-index (GSI).** Every row in `fleet-config` carries a `kind` (`repo`, `identity`, `notif_sub`, `trigger_rule`, `slack_workspace`, `slack_channel`, …), and every reader wants "all rows of one kind". A `kind-index` GSI (partition `kind`, sort `pk`) makes each such read a bounded **Query** — the dispatch hot path (identity/trigger-grant/repo snapshots) and the admin listers alike — instead of a full-table `Scan` + `FilterExpression` that reads and bills O(table) and grows with the whole fleet's data. `pk` is the index sort key so team-scoped kinds (`slack_channel#<team>#…`, `notif_sub#…`) range-Query by pk prefix. The one paged-Query helper lives in `config_query.query_kind` (dispatch) / `config_store._query_kind` (dashboard); it indexes the existing `kind` attribute, so no write path changed.
