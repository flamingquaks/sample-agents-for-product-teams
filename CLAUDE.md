...
- **Triggers → dispatch**: GitHub App webhook (`infra/dispatch/github_webhook.py`) and Asana webhook (`asana_webhook.py`) verify HMAC signatures and async-invoke the Dispatch Router. No per-repo GitHub Actions workflow. Both share `infra/dispatch/mentions.py` for HMAC verification + registry-backed @mention resolution (agent ids/aliases from the live registry, short-TTL cached), so a UI-onboarded agent is mentionable from every source with no per-receiver code change — a new trigger source is a thin adapter over this module. (Asana's assignment + custom-field paths stay built-in-only: they key on Asana-side bot accounts / dropdown enums that onboarding can't provision.)
