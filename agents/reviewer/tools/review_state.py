...
def get_review_state(repo: str, pr_number: int) -> str:
    ...
    return (
        ...
        "reviewing anything:\n\n"
        f"1. Retrieve the review ledger from AgentCore Memory namespace `{ns}` "
        "(use your memory retrieve tool). It holds: `last_reviewed_sha`, the list "
        "of prior finding `fingerprints`, and any `resolved` fingerprints (human "
        "dismissed/resolved). If there is no ledger, this is a first review.\n"
...
def record_review(repo: str, pr_number: int, head_sha: str, finding_count: int = 0) -> str:
    ...
    return (
        ...
        f"namespace `{ns}` (use your memory store tool):\n\n"
