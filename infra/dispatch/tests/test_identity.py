...
def test_enrichment_backfills_email_and_slack_handle(identity):
    first = identity.resolve(source="github", handle="jane-gh")
    identity.reset_cache()
    # Later she speaks in Slack, which reveals her email → same person, enriched.
    identity.resolve(
...
def test_auto_merge_on_email_match(identity):
    ...
    identity.reset_cache()
    # Slack-first with an email → new record.
    sl = identity.resolve(source="slack", handle="U1", workspace="T0ACME", email="jane@acme.com")
    assert sl.identity_id != gh.identity_id
    identity.reset_cache()
    # Now GitHub touch reveals the SAME email → merge the two records.
    merged = identity.resolve(source="github", handle="jane-gh", email="jane@acme.com")
