...
The SPA has three views, navigable via in-memory state (no router dependency):
...
All require a valid Cognito token with `operators` group membership.
...
To grant a user access to the dashboard:

1. Create the user in the dashboard's Cognito user pool (the `DashboardUserPoolId` output).
2. Add the user to the **`operators`** group. Without this group membership, the API returns 403 and the UI shows a friendly error banner.
3. Share the dashboard URL (`DashboardUrl` stack output) with the user.

Admin operations use the AWS Console (Cognito → User Pools → the dashboard pool → Groups → `operators` → Add user) or the AWS CLI:
