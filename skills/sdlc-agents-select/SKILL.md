...
|---|---|---|---|---|
| `workitems` | shipping | PO/PM. Decomposes feature asks into tracker issues; status reports; risk detection. | PM tool **and** source control (GitHub can satisfy **both** at once — see note below) | Slack (supported) for weekly status — any agent can also post results there |
...
## GitHub can be both PM and source control
...
- `pm: asana` — Asana is the PM backend (works exactly as before).
- `pm: github` — GitHub Issues + Projects V2 is the PM backend. In this case the
  same GitHub connection covers both roles, and you also record the Projects V2
  board number (see schema below). The agent runs with `PM_BACKEND=github`.
